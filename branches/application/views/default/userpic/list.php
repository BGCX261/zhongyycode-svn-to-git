...
.tis{margin:15px 0;}
p.img{ clear:both;margin:7px 0 0 6px;height:130px;width:130px; text-align:center;}
...
<div id="body">
  ...
  <div id="content">
    ...
    <p  class="huang" style="width:400px;float:left;">当前位置：<a href="/category/list">根目录</a>&gt <a href="/userpic/list?cate_id=<?php echo $cate_id; ?>"><?= $cate_rows['cate_name'] ;?></a><?=($recycle > 0) ? '回收站': '';?></p>
    <p style="width:200px; float:right; text-align:right;"><a href="/userpic/list?recycle=1">回收站</a></p>
    ...
    <fieldset>
        <legend> 搜索图片</legend>
                ...
                <?=($recycle > 0) ? '<input type="hidden" value="1"  name="recycle" />': '';?>
                照片名
    ...
    <form action="" method="post"  id="picList">
        ...
        <ul class="cate_list yoxview">
            ...
            <li>
                <p class="img"><a href="/<?php echo $item['id']?>.html" target="__blank"> <img src="<?php echo 'http://' . $item['disk_domain'] .'.wal8.com/' . $thumb->create($item['disk_id']. '/'.$item['userid'] .'/' . $item['picname'], 130,130); ?>" /></a></p>
                <br />
                ...
                <table>
                    <tr>
                        <td><input name="text" type="text"  size="22" value="<?php echo $item['custom_name']?>" /></td>
                    </tr>
                    <tr>
                        <td><input type="checkbox" name="id[]" value="<?=$item['id']?>" />
                            <div style="display:none;" id="zoom_<?=$key?>"><?=$item['picname']?></div>
                        ...
                        <div style="display:none;" id="pic_rule_<?php echo $key?>"><?php echo 'http://' . $item['disk_domain'] .'.wal8.com/' . $thumb->create($item['disk_id']. '/'.$item['userid'] .'/' . $item['picname'], 640,640, 's'); ?></div>
                        ...
                        <a  href="<?php if($recycle > 0) { ?>/userpic/delPic?id=<?php echo $item['id']?><?php } else {?>/userpic/recycle?id=<?php echo $item['id']?><?php } ?>" class="delete" title="删除"><img src="/images/user/icon_22.gif" /></a>
                        <?php if($recycle > 0) { ?><a  href="/userpic/recycle?id=<?php echo $item['id']?>&recycle=0"  title="恢复"><img src="/images/icon/arrow_undo.png" /></a><?php }?>
                        </td>
                </tr>
                </table>
            </li>
