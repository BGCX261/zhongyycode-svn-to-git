...
.user_zhannei_fa2{width:660px ;margin:00px 0 0 10px;padding-top:50px;}
.user_zhannei_fa2 .bline {border-bottom:1px dashed #CCCCCC;}
.user_zhannei_fa2 p {height:30px;line-height:30px;}
.fright {float:right;}
  ...
  <div id="content">
    ...
    <div class="tis"><img src="/images/user/user_zhannei.gif"></div>
     <ul class="nav_list">
        ...
        <li  class="current"><a href="/message/send">发件箱</a> </li>
    ...
    <div class="inner_content" style="padding:50px 0 0 10px; text-align:left;">
        <?php if (empty($info)) { ?>
        <table cellpadding="10"  width="650" cellspacing="0" border="0" align="left">
        <tr>
        ...
        <tr>
            ...
            <td width="100"><a href="/message/send?msg_id=<?=$item['msg_id']?>"><?php echo $item['title']?></a></td>
            <td class="td4"><?php echo date('Y-m-d H:i:s', $item['msg_time']);?></td><td class="td4 huang"><?php echo ($item['unread'] == 1) ? '<font color="green">未发送</font>' : '<font color="red">已发送</font>'?></td>
        </tr>
        <?php } ?>
        </table>
        <?php } else { ?>
        <div class="user_zhannei_fa2">
                <p class="bline"><span class="fright">发件时间：<?=date('Y-m-d H:i:s', $info['msg_time'])?></span>发件人：<?=$info['username']?><img src="images/icon_14.gif">  </p>
            <p class="bline">标 &nbsp; 题：<?=$info['title']?></p>
            <div>内 &nbsp; 容：<?=$info['content']?></div>
            <div class="tcent">
            <a href="/message/send"><img src="/images/user/icon_15.gif"></a> &nbsp;
            <a href="/message/del?msg_id=<?=$info['msg_id']?>"><img src="/images/user/icon_16.gif"></a></div>
        </div>
        <?php } ?>
    </div>
