...
<div id="body">
    ...
    <div id="main">
        <div class="mod_editor articles">
            <div class="title">
                <h2><span class="icon"></span>图片目录管理</h2>
                <span class="back"> <a href="/category/list">查看图片</a>&nbsp;&nbsp;&nbsp;&nbsp;  </span> </div>
            <div class="main">
                ...
                <div class="bd">
                    <table width="100%" cellspacing="0" style="margin-left:10px;">
                      <tr>
                        ...
                        d = new dTree('d');
                        d.add(0, -1, '<?=$auth['username']?>的图片目录');
                        <? foreach ($categories as $item) { ?>
                        d.add(<?=$item['cate_id']?>, <?=$item['parent_id']?>, '<?=$item['cate_name']?>[<?=$item['img_num']?>]', '/category/manage?cate_id=<?=$item['cate_id']?>', '<?=$item['cate_name']?>');
                        <? } ?>
                        document.write(d);
                        </script></div></td>
                        <td valign="top" align="left"><div style="margin-left:15px;width:400px;">
                        <form action="/category/cate<?=$cateInfo?'edit':'add'?>" method="post">
                        ...
                        <p style="margin:10px 0 0 88px;">
                          ...
                          <? if ($cateInfo) { ?><input type="button" value="删除分类" class="botton" onclick="cateDel('<?=$cateInfo['cate_id']?>');" /><input type="hidden" name="cate_id" value="<?=$cateInfo['cate_id']?>" /> &nbsp; <a href="/category/manage">添加分类</a><? } ?>
                        </p>
                        </form>
                        </div></td>
                      </tr>
...
function cateDel(id) {
    if (confirm('确认要删除该分类吗？')) {
        ...
        $.get('/category/catedel', {cate_id : id}, function(text){
            if (text == 'succeed') {
                alert('分类删除成功！');
                location.replace('/category/manage');
            } else {
                alert(text);
            }
        })
